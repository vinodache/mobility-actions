...
export async function run() {
  try {
    ...
    const inAppUpdatePriority = core.getInput("inAppUpdatePriority", {
      required: false,
    });
    ...
    const changesNotSentForReview =
      core.getInput("changesNotSentForReview", { required: false }) == "true";
    ...
    await validateStatus(
      status,
      userFractionFloat != undefined && !isNaN(userFractionFloat),
    );
    ...
    const validatedReleaseFiles: string[] = await validateReleaseFiles(
      releaseFiles ?? [releaseFile],
    );

    if (
      whatsNewDir != undefined &&
      whatsNewDir.length > 0 &&
      !fs.existsSync(whatsNewDir)
    ) {
    ...
    if (
      ...
      debugSymbols.length > 0 &&
      !fs.existsSync(debugSymbols)
    ) {
