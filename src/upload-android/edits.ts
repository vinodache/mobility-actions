...
async function uploadToPlayStore(
  ...
  if (options.track === "internalsharing") {
    ...
    const versionCodes = await uploadReleaseFiles(
      appEditId,
      options,
      releaseFiles,
    );
    ...
    for (const versionCode of versionCodes) {
      const url = inferInternalSharingDownloadUrl(
    ...
    if (res.data.id) {
      ...
      core.setOutput(
        "commitedEditIdExpiryTimeSeconds",
        res.data.expiryTimeSeconds,
      );

      core.exportVariable("COMMITED_EDIT_ID", res.data.id);
      core.exportVariable(
        "COMMITED_EDIT_ID_EXPIRY_IN_TIME_SECONDS",
        res.data.expiryTimeSeconds,
      );
...
async function uploadInternalSharingRelease(
  options: EditOptions,
  releaseFile: string,
): Promise<string> {
  ...
  if (releaseFile.endsWith(".apk")) {
    ...
    throw Error(
      `${releaseFile} is invalid (missing or invalid file extension).`,
    );
  }
  ...
  console.log(
    `${releaseFile} uploaded to Internal Sharing, download it with ${res.downloadUrl}`,
  );

  return res.downloadUrl;
}

async function validateSelectedTrack(
  appEditId: string,
  options: EditOptions,
): Promise<void> {
  ...
  if (allTracks.find((value) => value.track == options.track) == undefined) {
    ...
    throw Error(
      `Track "${options.track}" could not be found. Available tracks are: ${allTrackNames.toString()}`,
    );
  }
}
...
async function uploadDebugSymbolsFile(
  appEditId: string,
  versionCode: number,
  options: EditOptions,
) {
...
async function internalSharingUploadApk(
  options: EditOptions,
  apkReleaseFile: string,
): Promise<InternalAppSharingArtifact> {
  core.debug(
    `[packageName=${options.applicationId}]: Uploading Internal Sharing APK @ ${apkReleaseFile}`,
  );
...
async function internalSharingUploadBundle(
  options: EditOptions,
  bundleReleaseFile: string,
): Promise<InternalAppSharingArtifact> {
  core.debug(
    `[packageName=${options.applicationId}]: Uploading Internal Sharing Bundle @ ${bundleReleaseFile}`,
  );
...
async function uploadApk(
  ...
  core.debug(
    `[${appEditId}, packageName=${options.applicationId}]: Uploading APK @ ${apkReleaseFile}`,
  );
...
async function uploadBundle(
  ...
  core.debug(
    `[${appEditId}, packageName=${options.applicationId}]: Uploading App Bundle @ ${bundleReleaseFile}`,
  );
...
async function getOrCreateEdit(options: EditOptions): Promise<string> {
  ...
  core.debug(
    `This new edit expires at ${String(insertResult.data.expiryTimeSeconds)}`,
  );
